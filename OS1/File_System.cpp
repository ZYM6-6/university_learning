...
void FileSystem::Update() {
	Buf* pBuf;
	superBlock->s_fmod = 0;
	superBlock->s_time = (int)Common::time(NULL);
	for (int j = 0; j < 2; j++) {
		int* p = (int *)superBlock + j * 128;
		pBuf = this->bufferManager->GetBlock(FileSystem::SUPERBLOCK_START_SECTOR + j);
		Common::memcpy(pBuf->Address, p, BLOCK_SIZE);
		this->bufferManager->Buf_Write(pBuf);
	}
	g_INodeTable.UpdateINodeTable();
	this->bufferManager->Buf_Flush();
}
...
Buf* FileSystem::Alloc() {
	...
	*/
	if (superBlock->s_nfree <= 0) {
		pBuf = this->bufferManager->Buf_Read(blkno);
		int* p = (int *)pBuf->Address;
		superBlock->s_nfree = *p++;
		memcpy(superBlock->s_free, p, sizeof(superBlock->s_free));
		this->bufferManager->Buf_Relse(pBuf);
	}
	pBuf = this->bufferManager->GetBlock(blkno);
	if (pBuf) {
		this->bufferManager->Buf_Clear(pBuf);
	}
	superBlock->s_fmod = 1;
	return pBuf;
}
...
INode* FileSystem::IAlloc() {
	...
	if (superBlock->s_ninode <= 0) {
		...
		for (int i = 0; i < superBlock->s_isize; ++i) {
			pBuf = this->bufferManager->Buf_Read(FileSystem::INODE_ZONE_START_SECTOR + i);
			int* p = (int*)pBuf->Address;
...
void FileSystem::Free(int blkno) {
	...
	if (superBlock->s_nfree >= SuperBlock::MAX_NFREE) {
		...
		*p++ = superBlock->s_nfree;
		Common::memcpy(p, superBlock->s_free, sizeof(int)*SuperBlock::MAX_NFREE);
		superBlock->s_nfree = 0;
		this->bufferManager->Buf_Write(pBuf);
	}
