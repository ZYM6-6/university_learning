...
User::User() {
	...
	pdir = NULL;
	Common::memset(arg, 0, sizeof(arg));
}
...
void User::U_Create(string fileName, string mode) {
...
void User::U_Open(string fileName, string mode) {
	if (!checkPathName(fileName)) {
		return;
	}
...
void User::U_Seek(string sfd, string offset, string origin) {
	if (sfd.empty() || !isdigit(sfd.front())) {
		cout << "parameter fd can't be empty or be nonnumeric ! \n";
		return;
	}
